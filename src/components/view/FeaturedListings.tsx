  ...
  return (
    <section className="bg-white py-12 px-4">
      <div className="max-w-6xl mx-auto text-center">
        <h2 className="title-header-text mb-4">Check Our Featured Listings</h2>
        <p className="title-para-text mb-12">Lorem ipsum dolor sit amet, consectetur adipiscing</p>

        <div className="flex justify-end gap-2 mb-4 pr-2">
          <button onClick={handlePrev} className="bg-gray-200 text-sm px-3 py-1 rounded hover:bg-gray-300">Prev</button>
          <button onClick={handleNext} className="bg-gray-200 text-sm px-3 py-1 rounded hover:bg-gray-300">Next</button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {visibleListings.map((listing) => (
            <Link key={listing.id} href={`/property/${listing.id}`}>
              <div className="cursor-pointer bg-white rounded-md shadow-sm hover:shadow-md transition">
                <div className="relative h-64 w-full">
                  <Image src={listing.img} alt={listing.title} fill className="object-cover rounded-t-md" />
                  <div className="absolute top-3 left-3">
                    <span className="bg-green-500 text-white text-xs font-semibold px-2 py-1 rounded">FEATURED</span>
                  </div>
                  <div className="absolute top-3 right-3">
                    <span className="bg-black text-white text-xs font-semibold px-2 py-1 rounded">FOR RENT</span>
                  </div>
                  <div className="absolute bottom-3 left-3 text-white font-semibold text-lg">{listing.price}</div>
                ...
                <div className="p-4">
                  <h3 className="text-start font-semibold text-gray-800 mb-1">{listing.title}</h3>
